...
local XRayPlugin = (ok_wc and WidgetContainer and WidgetContainer.extend)
		and WidgetContainer:extend({
			name = "xray",
			is_doc_only = true,
		})
	or { name = "xray_failed" }
...
function XRayPlugin:init()
	local ok, err = pcall(function()
		...
		pcall(function()
			...
			if ok_order and reader_menu_order and reader_menu_order.tools then
				for i, v in ipairs(reader_menu_order.tools) do
					if v == "xray" then
						table.remove(reader_menu_order.tools, i)
						break
					end
				end
		...
		local legacy_files =
			{ "aihelper.lua", "cachemanager.lua", "chapteranalyzer.lua", "lookupmanager.lua", "updater.lua" }
		...
		self.auto_fetch_enabled = not (
			self.ai_helper.settings and self.ai_helper.settings.auto_fetch_on_chapter == false
		)
...
function XRayPlugin:onDictButtonsReady(dict_popup, dict_buttons)
	...
	local xray_button = {
		...
		callback = function()
			btn.callback(nil)
		end, -- nil => uses dict_popup upvalue
	}
...
function XRayPlugin:onReaderReady()
	...
	UIManager:scheduleIn(1.5, function()
		...
		if self.mountUnderlineOverlay then
			self:mountUnderlineOverlay()
		end
		if self.mountTapHandler then
...
function XRayPlugin:onPageUpdate(pageno)
	...
	if page_interval and page_interval > 0 then
		...
		self:log(
			"XRayPlugin: Ultra mode page interval crossed. Page: "
				.. tostring(pageno)
				.. ", Last: "
				.. tostring(last)
				.. ", Diff: "
	...
	if self:isNonNarrativeChapter(chapter_title) then
		if not self.chapters_fetched[unique_id] then
			self:log(
				"XRayPlugin: Skipping non-narrative chapter: "
					.. tostring(chapter_title)
					.. " (page "
					.. tostring(chapter_page)
					.. ")"
			)
	...
	if is_populated then
		if not self.chapters_fetched[unique_id] then
			self:log(
				"XRayPlugin: Chapter already populated in data: "
					.. tostring(chapter_title)
					.. " (page "
					.. tostring(chapter_page)
					.. ")"
	...
	if unique_id == self.last_auto_chapter then
		return
	end
				...
				sub_item_table = {
					{
						...
						checked_func = function()
							local val = self.ai_helper
								and self.ai_helper.settings
								and self.ai_helper.settings.ui_popup_intext
							if val == nil then
								return true
							end
							return val
					...
					},
					{
						...
						checked_func = function()
							local val = self.ai_helper
								and self.ai_helper.settings
								and self.ai_helper.settings.ui_popup_menu
							if val == nil then
								return false
							end
							return val
					...
					},
					{
						...
						keep_menu_open = true,
						callback = function()
							self:showLinkedEntriesSettings()
						end,
					},
					{
				...
				sub_item_table = {
					{
						...
						sub_item_table = {
							...
							},
							{
								...
								keep_menu_open = true,
								callback = function()
									self:showAutoDupeCheckSettings()
								end,
							},
						},
					},
						...
						sub_item_table = {
							{
								...
								keep_menu_open = true,
								callback = function()
									self:showEntityLengthPresets("char_desc_len", self.loc:t("menu_characters"))
								end,
							},
							{
								text = self.loc:t("menu_locations") or "Locations",
								keep_menu_open = true,
								callback = function()
									self:showEntityLengthPresets("loc_desc_len", self.loc:t("menu_locations"))
								end,
							},
							{
								text = self.loc:t("menu_timeline") or "Timeline",
								keep_menu_open = true,
								callback = function()
									self:showEntityLengthPresets(
										"timeline_event_len",
										self.loc:t("menu_timeline"),
										true
									)
								end,
							},
							...
							},
							{
								...
								keep_menu_open = true,
								callback = function()
									self:showEntityLengthPresets("term_def_len", self.loc:t("menu_terms") or "Glossary")
								end,
							},
						},
					},
						...
						sub_item_table = {
							{
								...
								checked_func = function()
									return self.ai_helper.settings.series_context_enabled
								end,
								callback = function()
									self:toggleSeriesContextEnabled()
								end,
							},
							{
				...
				sub_item_table = {
					{
						...
						keep_menu_open = true,
						sub_item_table_func = function()
							return self:getAIModelSelectionMenu("primary")
						end,
					},
					{
						...
						keep_menu_open = true,
						sub_item_table_func = function()
							return self:getAIModelSelectionMenu("secondary")
						end,
					},
					{
						...
						keep_menu_open = true,
						callback = function()
							self:showReasoningEffortSettings()
						end,
						separator = true,
					},
					{
	...
	refresh = function()
		...
		local function saveSetting(key, val)
			...
			local is_visual = (
				...
				or key == "unit_underline_thickness"
				or key == "unit_underline_intensity"
				or key == "unit_tooltip_timeout"
			)
...
function XRayPlugin:showUnitAutoScanCard()
	...
	XRaySettingsCard.show(self, {
		...
		about_text = self.loc:t("unit_auto_scan_about")
			or "Scanning can take up to 15-20 seconds for large books. This only happens the first time the book is opened, and the results are saved for the future.",
	})
end
...
function XRayPlugin:showUnitScanWrittenNumbersCard()
	...
	XRaySettingsCard.show(self, {
		...
		description = self.loc:t("unit_scan_written_numbers_desc")
			or "Scan for spelled-out numbers (e.g. 'five miles'):",
		options = {
			{ text = enabled_text, value = true },
			{ text = disabled_text, value = false },
		},
		...
		about_text = self.loc:t("unit_scan_written_numbers_about")
			or "Scanning written-out numbers (like 'five miles') requires a second full-text pass. Skipping this pass on lower-powered devices (Kindle, Kobo, PocketBook) provides a 4x to 5x scan speedup and prevents startup freezes. On faster platforms like Android or desktops, there is no noticeable slowdown.",
	})
end
...
findUnitConverterMenuPath = function(self)
	...
	for tab_idx, tab_items in ipairs(tab_item_table) do
		for item_idx, item in ipairs(tab_items) do
			self:log(
				string.format(
					"XRayPlugin: findUnitConverterMenuPath checking tab=%d item=%d id=%s text=%s",
					tab_idx,
					item_idx,
					tostring(item.id),
					tostring(item.text)
...
function XRayPlugin:showUnitConverterNewFeatureCard()
	...
	local function renderCard()
		...
		local description_box = TextBoxWidget:new({
			text = self.loc:t("unit_conv_new_feature_desc")
				or "X-Ray now detects measurements (lengths, weights, temperature) in your books and highlights them with a subtle underline. Tap any highlighted unit to see its converted value in a popup tooltip.\n\nNote: A scan is required the first time a book is opened.",
			face = Font:getFace("cfont", ui_font_size),
			width = dialog_w - sc(32),
...
function XRayPlugin:detectBookTypeHeuristic()
	...
	-- 1. Niche checks
	if match_keywords(subjects, manga_kw) then
		return "manga", true
	end
	if match_keywords(title, manga_kw) or match_keywords(filename, manga_kw) then
		return "manga", false
	end

	if match_keywords(subjects, poetry_kw) then
		return "poetry", true
	end
	if match_keywords(title, poetry_kw) or match_keywords(filename, poetry_kw) then
		return "poetry", false
	end

	if match_keywords(subjects, cookbook_kw) then
		return "cookbook", true
	end
	if match_keywords(title, cookbook_kw) or match_keywords(filename, cookbook_kw) then
		return "cookbook", false
	end

	if match_keywords(subjects, textbook_kw) then
		return "textbook", true
	end
	if match_keywords(title, textbook_kw) or match_keywords(filename, textbook_kw) then
		return "textbook", false
	end

	if match_keywords(subjects, travel_kw) then
		return "travel", true
	end
	if match_keywords(title, travel_kw) or match_keywords(filename, travel_kw) then
		return "travel", false
	end

	-- 2. Fiction / Non-Fiction positive checks
	if match_keywords(subjects, fiction_kw) then
		return "prose_fiction", true
	end
	if match_keywords(title, fiction_kw) or match_keywords(filename, fiction_kw) then
		return "prose_fiction", false
	end

	if match_keywords(subjects, nonfiction_kw) then
		return "prose_nonfiction", true
	end
	if match_keywords(title, nonfiction_kw) or match_keywords(filename, nonfiction_kw) then
		return "prose_nonfiction", false
	end
...
function XRayPlugin:triggerBookTypeDetection()
	...
	local function checkAndTriggerScan()
		if has_unit_cache then
			...
			if self.applyUnitUnderlines then
				self:applyUnitUnderlines()
			end
		else
			-- Check if current type is disabled before scanning
			local settings = self.ai_helper and self.ai_helper.settings or {}
			local disabled_types = settings.unit_disabled_book_types
		...
		if is_confident then
			...
			if pid then
				local function pollResult()
					...
					elseif type(res) == "table" and res.book_type_label then
						self:log(
							"XRayPlugin: Book type AI refinement complete! Result: " .. tostring(res.book_type_label)
						)
						cached.book_type_label = res.book_type_label
						cached.book_type_detected_by_ai = true
						self.cache_manager:asyncSaveCache(self.ui.document.file, cached)
...
function XRayPlugin:showBookTypeOverrideCard(refresh_parent)
	...
	local book_types = {
		...
		{
			key = "prose_nonfiction",
			text = self.loc:t("unit_book_type_prose_nonfiction") or "Non-Fiction (History, Science, etc.)",
		},
			...
			sub_item_table_func = function()
				...
				for _, bt in ipairs(book_types) do
					table.insert(opts, {
						text = bt.text,
						checked_func = function()
							local disabled = self.ai_helper.settings.unit_disabled_book_types
								or { "manga", "graphic_novel", "children", "poetry" }
							local disabled_map = {}
