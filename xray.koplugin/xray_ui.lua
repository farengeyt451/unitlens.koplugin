...
local function _isCJKFontFamily(family)
	...
	return fl:find("cjk")
		or fl:find("han")
		or fl:find("wenquanyi")
		or fl:find("noto.*jp")
		or fl:find("noto.*sc")
		or fl:find("noto.*tc")
		or fl:find("noto.*kr")
		or fl:find("source han")
		or fl:find("adobe")
		or fl:find("kozuka")
		or fl:find("hiragino")
		or fl:find("meiryo")
...
function XRayBottomPopup:init()
	...
	if e.aliases then
		...
		local src = (type(e.aliases) == "table") and e.aliases or (type(e.aliases) == "string") and { e.aliases } or {}
	...
	if aliases_str then
		...
		table.insert(
			vg_components,
			make_text(get_loc_t("label_aliases", "ALIASES") .. ": " .. aliases_str, face_small_normal, "justify")
		)
		has_metadata = true
	end
	...
	local linked_enabled = plugin
		and plugin.ai_helper
		and plugin.ai_helper.settings
		and plugin.ai_helper.settings.linked_entries_enabled ~= false
	local related = {}
	if linked_enabled and plugin and plugin.findRelatedEntities then
		related = plugin:findRelatedEntities(desc_str or "", e.name) or {}
	end
	local mentions_enabled = plugin
		and plugin.ai_helper
		and plugin.ai_helper.settings
		and plugin.ai_helper.settings.mentions_enabled ~= false
		...
		local read_more_btn = make_btn(get_loc_t("read_more", "Read More"), function()
			...
			if aliases_str then
				full_text_for_viewer = full_text_for_viewer
					.. get_loc_t("label_aliases", "Aliases")
					.. ": "
					.. aliases_str
					.. "\n"
			end
	...
	if mentions_enabled and not e.is_timeline and not e.is_conversion then
		local right_btn = make_btn(get_loc_t("find_mentions", "Find Mentions"), function()
			UIManager:close(self)
			if plugin then
				if plugin.showMentionsForEntity then
					plugin:showMentionsForEntity(e)
				elseif plugin.findMentions then
					plugin:findMentions(e)
				elseif plugin.showMentions then
					plugin:showMentions(e)
				end
			end
		end)
...
local function shouldUseBottomPopup(plugin, opts)
	...
	if ui_popup_intext == nil then
		ui_popup_intext = true
	end
	local ui_popup_menu = settings.ui_popup_menu
	if ui_popup_menu == nil then
		ui_popup_menu = false
	end
...
function M:showLanguageSelection()
	...
	local function changeLang(lang_code)
		...
		local msg = (self.loc and self.loc:t("language_changed_reopen"))
			or "Language changed. Reopen the menu to see the changes."
	...
	local items = {
		{
			text = (settings_lang == "auto" and "[✓] " or "[  ] ")
				.. (self.loc:t("lang_follow_system") or "Automatic (Follow System)"),
			callback = function()
				changeLang("auto")
			end,
		},
		{
			text = (settings_lang == "book" and "[✓] " or "[  ] ")
				.. (self.loc:t("lang_follow_book") or "Automatic (Follow Book)"),
			callback = function()
				changeLang("book")
			end,
			separator = true,
	...
	local langs = self.loc and self.loc.available_languages
		or { "en", "de", "fr", "ru", "zh_CN", "tr", "pt_br", "es", "uk", "hu" }
	...
	elseif code == "book" then
		if self.ui and self.ui.document then
			...
			if book_lang then
				...
				if book_lang:lower():find("zh") then
					lang = "zh_CN"
				elseif book_lang:lower():find("pt") then
					lang = "pt_br"
				elseif book_lang:lower():find("ja") or book_lang:lower():find("jp") then
					lang = "ja"
				end
				if supported[lang] then
					return lang
...
function M:isXRayUIActive()
	return self._menu_creating
		or self.xray_menu
		or self.char_menu
		or self.loc_menu
		or self.timeline_menu
		or self.hf_menu
		or self.terms_menu
		or self.ldlg
		or self.active_related_menu
		or self.length_presets_menu
end
...
function M:applyLanguageLogic()
	...
	self:log(
		"XRayPlugin: Applying language logic. Settings: "
			.. tostring(settings_lang)
			.. ", Resolved: "
			.. tostring(resolved)
	)
...
function M:checkBookLanguageMatch()
	...
	local props = self.ui.document:getProps()
	local book_lang = props.language
	if not book_lang or book_lang == "" then
		return
	end

	local lang = book_lang:sub(1, 2):lower()
	if book_lang:find("zh") then
		lang = "zh_CN"
	elseif book_lang:find("pt") then
		lang = "pt_br"
	elseif book_lang:find("ja") or book_lang:find("jp") then
		lang = "ja"
	end
	...
	if self.suggestion_dismissed[self.ui.document.file] then
		return
	end
	...
	if cache and cache.ignore_lang_mismatch then
		return
	end
...
function M:showCharacters()
	...
	if #self.characters > 0 then
		...
		table.insert(items, {
			text = "⋈ " .. (self.loc:t("merge_duplicates") or "Merge Duplicates..."),
			callback = function()
				local ButtonDialog = require("ui/widget/buttondialog")
				local merge_dialog
	...
	table.insert(items, {
		text = "✚ " .. (self.loc:t("menu_fetch_more_chars") or "Fetch More Characters"),
		keep_menu_open = true,
		callback = function()
			self:fetchMoreCharacters()
		end,
		separator = #self.characters > 0,
	})
	for _, char in ipairs(self.characters) do
		...
		table.insert(items, {
			...
			separator = true,
			callback = function()
				self:showCharacterDetails(char, { source = "menu" })
			end,
		})
	end
	...
	UIManager:scheduleIn(0.3, function()
		...
		if
			...
			and self.pending_duplicate_review.characters
			and #self.pending_duplicate_review.characters > 0
		then
			...
			if #pairs > 0 then
				...
				UIManager:show(ConfirmBox:new({
					text = string.format(
						self.loc:t("pending_duplicates_prompt")
							or "AI found %d possible duplicate character(s) from the last fetch. Review now?",
						#pairs
					),
					...
					ok_callback = function()
						self:log(
							"XRayPlugin: User chose to review pending "
								.. tostring(#pairs)
								.. " duplicate(s) for characters"
						)
...
function M:findRelatedEntities(text, exclude_name)
	...
	local function isTooGeneric(term, entity_name)
		...
		if #term < 2 or honorifics[term_l] then
			return true
		end
...
function M:showCharacterDetails(character, opts)
	...
	local linked_enabled = self.ai_helper
		and self.ai_helper.settings
		and self.ai_helper.settings.linked_entries_enabled ~= false
	local related = linked_enabled and self:findRelatedEntities(resolved_desc or "", character.name) or {}
	local mentions_enabled = self.ai_helper
		and self.ai_helper.settings
		and self.ai_helper.settings.mentions_enabled ~= false
		...
		table.insert(buttons, 1, {
			{
				...
				callback = function()
					...
					if #meaningful_aliases > 0 then
						full_text = full_text
							.. (self.loc:t("label_aliases") or "Aliases")
							.. ": "
							.. table.concat(meaningful_aliases, ", ")
							.. "\n"
					end
					full_text = full_text .. "\n" .. resolved_desc
					if character.ai_reasoning then
...
function M:showTermDetails(term, opts)
	...
	if opts and opts.low_confidence then
		local warning = self.loc:t("low_conf_match", term.name)
			or string.format(
				"Partial match - showing '%s' for your query. Tap below to fetch the exact term.",
				term.name
			)
		table.insert(vg_components, VerticalSpan:new({ width = math.max(6, math.floor(fs * 0.3)) }))
		table.insert(
	...
	local function get_relookup_row()
		...
		return {
			{
				text = self.loc:t("relookup_button", safe_text) or ("Re-lookup '" .. safe_text .. "'"),
				callback = function()
					if self.active_details_dialog then
						UIManager:close(self.active_details_dialog)
						self.active_details_dialog = nil
...
function M:showTerms()
	...
	if #self.terms > 0 then
		table.insert(items, {
			text = "⌕ " .. (self.loc:t("search_term") or "Search Terms"),
			callback = function()
				self:showTermSearch()
			end,
		})
	...
	table.insert(items, {
		text = "✚ " .. (self.loc:t("menu_fetch_more_terms") or "Fetch More Terms"),
		callback = function()
			self:fetchMoreTerms()
		end,
		separator = #self.terms > 0,
	})
...
function M:findTermByName(word)
	if not word or not self.terms then
		return nil
	end
...
function M:showTermSearch()
	if not self.terms or #self.terms == 0 then
		UIManager:show(InfoMessage:new({ text = self.loc:t("no_terms_data"), timeout = 3 }))
		return
	end
		...
		buttons = {
			{
				...
				{
					text = self.loc:t("search_button") or "Search",
					is_enter_default = true,
					callback = function()
						...
						if search_text and #search_text > 0 then
							...
							if found then
								self:showTermDetails(found, { source = "menu" })
							else
								UIManager:show(
									InfoMessage:new({
										text = self.loc:t("term_not_found", search_text) or "Term not found.",
										timeout = 3,
									})
								)
							end
...
function M:showMentionsSettings()
	...
	XRaySettingsCard.show(self, {
		...
		description = self.loc:t("mentions_preference_desc")
			or "Select your preference for character and location mentions:",
		options = {
		...
		about_text = self.loc:t("mentions_setting_desc")
			or "Mentions scanning allows you to find every occurrence of a character or location in the book. This happens [B]automatically[/B] in the background to ensure the reader stays responsive.\n\nDisabling this will stop all background scanning and hide the [B]Find Mentions[/B] button.",
	})
end
...
function M:showAutoDupeCheckSettings()
	...
	XRaySettingsCard.show(self, {
		...
		description = self.loc:t("auto_dupe_check_preference_desc")
			or "Select your preference for automatic AI duplicate detection:",
		options = {
			{ text = enabled_text, value = true },
			{ text = disabled_text, value = false },
		},
...
function M:showLinkedEntriesSettings()
	...
	XRaySettingsCard.show(self, {
		...
		about_text = self.loc:t("linked_entries_setting_desc")
			or "Linked Entries automatically connects characters, locations, and historical figures when they are mentioned in each other's descriptions.\n\nDisabling this will hide the [B]Linked Entries[/B] button from detail dialogs.",
	})
end

function M:filterValidDuplicatePairs(list, pairs)
	if not pairs or not list then
		return {}
	end
	if not self.book_data then
		if not self.cache_manager then
		...
		if pair.primary and pair.secondary then
			...
			if not rejected_pairs[key] then
				...
				for _, it in ipairs(list) do
					if it.name then
						if it.name:lower() == p_name then
							primary_item = it
						end
						if it.name:lower() == s_name then
							secondary_item = it
						end
					end
				end
...
function M:walkDuplicatePairs(list, list_name, pairs_found)
	...
	self:log(
		"XRayPlugin: Walking "
			.. tostring(pairs_found and #pairs_found or 0)
			.. " duplicate pair(s) for "
			.. tostring(list_name)
	)
	...
	self:log(
		"XRayPlugin: "
			.. tostring(#pairs_found)
			.. " pair(s) remain after filtering rejected/non-existent for "
			.. tostring(list_name)
	)
	...
	local function processNextPair()
		if pair_idx > #pairs_found then
			saveAndRefresh()
			local msg = merge_count > 0 and self.loc:t("ai_merged_n", merge_count)
				...
				{
					{
						...
						callback = function()
							self:log(
								"XRayPlugin: Merging duplicate pair: keep '"
									.. tostring(pair.primary)
									.. "', remove '"
									.. tostring(pair.secondary)
									.. "'"
							)
							...
							if p_desc and s_desc then
								local wait_msg = InfoMessage:new({
									text = self.loc:t("merging_smartly") or "Merging...",
									timeout = 120,
								})
					...
					},
					{
						...
						callback = function()
							...
							UIManager:show(
								InfoMessage:new({
									text = self.loc:t("pair_rejected") or "Pair marked as not a duplicate.",
									timeout = 2,
								})
							)
							processNextPair()
					...
					},
					{
						...
						callback = function()
							self:log(
								"XRayPlugin: User stopped duplicate walk after " .. tostring(merge_count) .. " merge(s)"
							)
							UIManager:close(confirm_dialog)
...
function M:showAIFindDuplicatesFlow(list, list_name, entity_label)
	...
	local dup_book_text = self.chapter_analyzer:getTextForAnalysis(self.ui, 15000, nil, current_page)
	...
	UIManager:scheduleIn(0.1, function()
		...
		local pairs_found, err_code, err_msg =
			self.ai_helper:findDuplicates(title, author, list, entity_label, reading_percent, dup_book_text)
		UIManager:close(wait_msg)
		...
		self:log(
			"XRayPlugin: AI duplicate scan found " .. tostring(#pairs_found) .. " pair(s) for " .. tostring(list_name)
		)
				...
				table.insert(buttons, {
					{
						...
						callback = function()
							...
							local confirm = ConfirmBox:new({
								text = string.format(
									self.loc:t("merge_confirm")
										or "Merge %s into %s? The secondary entry will be deleted and its aliases absorbed.",
									secondary_name,
									primary_item.name
								),
								...
								ok_callback = function()
									...
									UIManager:scheduleIn(0.1, function()
										...
										if self.ai_helper and self.ai_helper:hasApiKey() then
											local sec_item = nil
											for _, it in ipairs(list) do
												if it.name == secondary_name then
													sec_item = it
													break
												end
											end
											...
											if sec_item and primary_item.description and sec_item.description then
												...
												ai_merged_desc = self.ai_helper:mergeDescriptionsWithAI(
										...
										if
											self:mergeEntries(list, primary_item.name, secondary_name, ai_merged_desc)
										then
											...
											if not self.book_data then
												self.book_data = self.cache_manager:loadCache(self.ui.document.file)
													or {}
											end
											local cache = self.book_data
											if list_name == "characters" then
	...
	XRaySettingsCard.show(self, {
			...
			return {
				{
					text = self.loc:t(
						"auto_update_ultra",
						self.ai_helper.settings and self.ai_helper.settings.auto_fetch_page_interval or 25
					)
						or (
							"Ultra: checks every "
							.. (self.ai_helper.settings and self.ai_helper.settings.auto_fetch_page_interval or 25)
				...
				{
					text = self.loc:t("auto_update_aggressive") or "Aggressive: checks every new chapter",
					value = "aggressive",
				},
				{
					text = self.loc:t("auto_update_balanced") or "Balanced: checks at most every 5 mins",
					value = "balanced",
				},
				{
					text = self.loc:t("auto_update_economical") or "Economical: checks at most every 15 mins",
					value = "economical",
				},
		...
		save_func = function(val, refresh_card)
			...
			elseif val == "aggressive" then
				self.auto_fetch_enabled = true
				self.ai_helper:saveSettings(
					{ auto_fetch_on_chapter = true, auto_fetch_cooldown = 0 },
					{ "auto_fetch_page_interval" }
				)
			elseif val == "balanced" then
				self.auto_fetch_enabled = true
				self.ai_helper:saveSettings(
					{ auto_fetch_on_chapter = true, auto_fetch_cooldown = 300 },
					{ "auto_fetch_page_interval" }
				)
			elseif val == "economical" then
				self.auto_fetch_enabled = true
				self.ai_helper:saveSettings(
					{ auto_fetch_on_chapter = true, auto_fetch_cooldown = 900 },
					{ "auto_fetch_page_interval" }
				)
			elseif val == "sparse" then
				self.auto_fetch_enabled = true
				self.ai_helper:saveSettings(
					{ auto_fetch_on_chapter = true, auto_fetch_cooldown = 1800 },
					{ "auto_fetch_page_interval" }
				)
...
function M:showSpoilerSettings()
	XRaySettingsCard.show(self, {
		...
		about_text = self.loc:t("spoiler_free_about")
			or "Spoiler-free mode limits AI extraction to the pages you have already read (up to your current page), preventing spoilers from future chapters.\n\n[B]Full Book Mode:[/B] Analyzes the entire book, which [B]may contain spoilers[/B].",
	})
end
...
function M:showEntityLengthPresets(setting_key, entity_name, is_timeline)
	...
	local presets = {
		{
			name = self.loc:t("desc_len_short"),
			val = is_timeline and 50 or (setting_key == "char_desc_len" and 80 or 50),
		},
		{
			name = self.loc:t("desc_len_default"),
			val = is_timeline and 80 or (setting_key == "char_desc_len" and 200 or 100),
		},
		{
			name = self.loc:t("desc_len_detailed"),
			val = is_timeline and 150 or (setting_key == "char_desc_len" and 350 or 200),
		},
		{
			name = self.loc:t("desc_len_v_detailed"),
			val = is_timeline and 200 or (setting_key == "char_desc_len" and 500 or 300),
		},
	}
...
function M:showAuthorInfo()
	if
		not self.author_info
		or not self.author_info.description
		or self.author_info.description == ""
		or self.author_info.description == (self.loc:t("msg_no_bio") or "No biography available.")
	then
		local ButtonDialog = require("ui/widget/buttondialog")
		local ask_dialog
		ask_dialog = ButtonDialog:new({
			title = (self.loc:t("menu_fetch_author") or "Fetch Author Info") .. "\n\n" .. (self.loc:t(
				"no_author_data_fetch"
			) or "No author biography available. Fetch now?"),
			buttons = {
				{
					{
						text = self.loc:t("cancel"),
...
function M:showLocations()
	...
	UIManager:scheduleIn(0.3, function()
		...
		if
			self.pending_duplicate_review
			and self.pending_duplicate_review.locations
			and #self.pending_duplicate_review.locations > 0
		then
			...
			if #pairs > 0 then
				...
				UIManager:show(ConfirmBox:new({
					text = string.format(
						self.loc:t("pending_duplicates_prompt")
							or "AI found %d possible duplicate location(s) from the last fetch. Review now?",
						#pairs
					),
					ok_text = self.loc:t("review") or "Review",
					cancel_text = self.loc:t("later") or "Later",
...
function XRayLogViewer:_rebuild()
	...
	local title_text = string.format(
		"%s (%d/%d)",
		(self.ui_instance and self.ui_instance.loc:t("menu_view_log")) or "X-Ray Log",
		current_page,
		total_pages
	)
	...
	if
		self.ui_instance
		and self.ui_instance.plugin
		and self.ui_instance.plugin.ui
		and self.ui_instance.plugin.ui.font
	then
	...
	if is_cjk then
		...
		content_face = Font:getFace("infont", fs) or Font:getFace("smallinfont", fs) or Font:getFace("cfont", fs)
	end
...
function XRayLogViewer:_reloadFromDisk()
	...
	if not snapshot or snapshot == "" then
		UIManager:show(InfoMessage:new({
			text = (self.ui_instance and self.ui_instance.loc:t("log_empty"))
				or "Log is empty or currently unavailable.",
			timeout = 3,
		}))
		return
	end
...
function M:toggleXRayMode()
	...
	XRaySettingsCard.show(self, {
		...
		save_func = function(val)
			...
			if self.ai_helper then
				self.ai_helper:saveSettings({ xray_mode_enabled = val })
			end
			UIManager:setDirty(nil, "ui")
		end,
	})
...
function M:showTimelineEventDetails(ev, opts)
	...
	if is_truncated then
		...
		if last_sp then
			display_text = display_text:sub(1, last_sp - 1)
		end
...
function M:showHistoricalFigureDetails(fig, opts)
	...
	if bio == "---" then
		bio = self.loc:t("msg_no_bio") or "No biography available."
	end
		...
		if prov_data then
			...
			table.insert(menu_items, {
				...
				sub_item_table_func = function()
					return self:getProviderKeySubMenu(p.id, p.name)
				end,
			})
		end
	end
...
function M:getProviderKeySubMenu(provider, provider_name)
	...
	local ui_key = (self.ai_helper and self.ai_helper.settings) and self.ai_helper.settings[provider .. "_api_key"]
		or ""
	...
	local menu_items = {
		{
			...
			checked_func = function()
				if not self.ai_helper or not self.ai_helper.providers or not self.ai_helper.providers[provider] then
					return false
				end
			...
			callback = function()
				...
				if provider:find("custom") then
					...
					local function promptModel(endpoint, key)
						local current_model = (self.ai_helper and self.ai_helper.settings)
								and self.ai_helper.settings[provider .. "_model"]
							or ""
						local model_dialog
						model_dialog = InputDialog:new({
							title = self.loc:t("custom_api_model_title", provider:sub(-1)),
							input = current_model,
							input_hint = self.loc:t("custom_api_model_hint")
								or "e.g., google/gemini-2.5-flash or openai/gpt-4o",
							buttons = {
								{
									{
										text = self.loc:t("cancel"),
									...
									},
									{
										...
										callback = function()
											...
											UIManager:show(
												InfoMessage:new({
													text = self.loc:t("custom_api_saved", provider:sub(-1)),
													timeout = 3,
												})
											)
											UIManager:setDirty(nil, "ui")
					...
					local function promptEndpoint()
						local current_endpoint = (self.ai_helper and self.ai_helper.settings)
								and self.ai_helper.settings[provider .. "_endpoint"]
						...
						endpoint_dialog = InputDialog:new({
							...
							input_hint = self.loc:t("custom_api_endpoint_hint")
								or "e.g., https://openrouter.ai/api/v1/chat/completions",
							buttons = {
								{
									{
										text = self.loc:t("cancel"),
		...
		table.insert(menu_items, {
			...
			checked_func = function()
				return (self.ai_helper and self.ai_helper.settings)
						and self.ai_helper.settings[provider .. "_is_reasoning"]
					or false
			end,
			callback = function()
				local current = (self.ai_helper and self.ai_helper.settings)
						and self.ai_helper.settings[provider .. "_is_reasoning"]
					or false
...
function M:getAIModelSelectionMenu(setting_type)
	...
	local function getCustomSlotModel(slot)
		local model = self.ai_helper
			and self.ai_helper.providers
			and self.ai_helper.providers[slot]
			and self.ai_helper.providers[slot].model
	...
	for _, p in ipairs(providers) do
		...
		table.insert(menu_items, {
			...
			keep_menu_open = true,
			checked_func = function()
				if not self.ai_helper or not self.ai_helper.settings then
					return false
				end
				local current = setting_type == "primary" and self.ai_helper.settings.primary_ai
					or self.ai_helper.settings.secondary_ai
			...
			sub_item_table_func = function()
				...
				for _, m in ipairs(provider_models) do
					...
					table.insert(sub_items, {
						text = model_id .. " [" .. (model_cost == "free" and self.loc:t("model_free") or self.loc:t(
							"model_paid"
						)) .. "]",
						checked_func = function()
							if not self.ai_helper or not self.ai_helper.settings then
	...
	table.insert(menu_items, {
		...
		callback = function()
			...
			local input_dialog
			local current = (self.ai_helper and self.ai_helper.settings)
					and (setting_type == "primary" and self.ai_helper.settings.primary_ai or self.ai_helper.settings.secondary_ai)
				or nil
			input_dialog = InputDialog:new({
				title = (self.loc:t("menu_custom_model_title") or "Custom %s Model"):format(
					setting_type:gsub("^%l", string.upper)
				),
						...
						},
						{
							...
							callback = function()
								...
								if custom_model and #custom_model > 0 then
									...
									if string.find(custom_model, "deepseek") then
										provider = "deepseek"
									end
									if string.find(custom_model, "claude") then
										provider = "claude"
									end
									if string.find(custom_model, "gemini") then
										provider = "gemini"
									end
									self.ai_helper:setUnifiedModel(setting_type, provider, custom_model)
									UIManager:show(
										InfoMessage:new({
											text = setting_type:gsub("^%l", string.upper)
												.. " AI set to "
												.. custom_model,
											timeout = 3,
										})
					...
					callback = function()
						...
						if search_text and #search_text > 0 then
							...
							if found_char then
								...
								UIManager:show(
									InfoMessage:new({
										text = self.loc:t("character_not_found", search_text),
										timeout = 3,
									})
								)
							end
...
function M:showConfigSummary()
	...
	XRaySettingsCard.showAbout(
		self,
		self.loc:t("menu_view_config") or "View All Config Values",
		table.concat(lines, "\n")
	)
end
...
function M:showBetaChannelSettings()
	...
	XRaySettingsCard.show(self, {
		...
		about_text = self.loc:t("beta_channel_desc")
			or "The beta channel allows you to receive pre-release versions of the X-Ray plugin. These versions include the latest features and bug fixes but may be [B]less stable[/B] than the regular release.",
	})
end
...
function M:showSeriesContextPrompt(series_info)
	...
	self:log(
		"XRayPlugin: Series: showSeriesContextPrompt: Series detected: "
			.. series_info.name
			.. ", index="
			.. tostring(series_info.index)
			.. ". Showing prompt dialog."
	)

	local body_text =
		self.loc:t("series_context_prompt_text", series_info.index, series_info.name, series_info.index - 1)
		...
		buttons = {
			{
				...
				},
				{
					...
					callback = function()
						...
						UIManager:close(confirm)
						local ask_later_msg = self.loc:t("series_ask_later_msg")
							or "Series recap postponed. We will ask again when you open/resume this book."
						UIManager:show(InfoMessage:new({
							text = ask_later_msg,
							timeout = 5,
						}))
				...
				},
				{
					...
					callback = function()
						...
						self.cache_manager:asyncSaveCache(self.ui.document.file, cache)
						local disabled_msg = self.loc:t("series_disabled_msg")
							or "Auto-prompt disabled for this book. You can manually fetch recap from X-Ray menu."
						UIManager:show(InfoMessage:new({
							text = disabled_msg,
							timeout = 5,
						}))
...
function M:checkSeriesContext()
	...
	self:log(
		"XRayPlugin: Series: checkSeriesContext: checking book title="
			.. tostring(title)
			.. ", author="
			.. tostring(author)
	)
	...
	if series_info and series_info.name and series_info.index then
		if series_info.index > 1 then
			self:log(
				"XRayPlugin: Series: Metadata check found series: "
					.. series_info.name
					.. ", index="
					.. tostring(series_info.index)
			)
			...
			self:log(
				"XRayPlugin: Series: Metadata check found series: "
					...
					.. ", index="
					.. tostring(series_info.index)
					.. " (first book). Caching check outcome."
			)
			saveSeriesChecked()
			return
		end
	...
	if not request_pid then
		self:log(
			"XRayPlugin: Series: Async check not supported/failed (e.g. on Windows). Skipping automatic AI fallback."
		)
		return
	end
		...
		elseif result == false then
			...
			if result.is_series then
				...
				if name and name ~= "" then
					...
					self:log(
						"XRayPlugin: Series: Async check detected series="
							.. tostring(name)
							.. ", index="
							.. tostring(index)
					)
					if index > 1 then
						...
						self:log(
							"XRayPlugin: Series: Book is first in series (index="
								.. tostring(index)
								.. "), skipping prompt. Caching check outcome."
						)
						saveSeriesChecked()
					end
				else
					self:log(
						"XRayPlugin: Series: Async check detected series, but name is invalid. Caching check outcome."
					)
					saveSeriesChecked()
				end
			else
				self:log(
					"XRayPlugin: Series: Async check determined book is not part of a series. Caching check outcome."
				)
				saveSeriesChecked()
			end
		end
