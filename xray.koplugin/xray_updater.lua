...
local _plugin_dir = (debug.getinfo(1, "S").source or ""):match("^@(.+)/[^/]+$") or "/mnt/us/extensions/xray.koplugin"

local function _apiUrl(use_beta)
	if use_beta then
		return string.format("https://api.github.com/repos/%s/%s/releases", GITHUB_OWNER, GITHUB_REPO)
	end
	return string.format("https://api.github.com/repos/%s/%s/releases/latest", GITHUB_OWNER, GITHUB_REPO)
end
...
local function _parseRelease(body, use_beta)
	...
	if not ok_j then
		...
		local download_url =
			body:match('"browser_download_url"%s*:%s*"([^"]*' .. ASSET_NAME:gsub("%.", "%%.") .. '[^"]*)"')
...
local function _applyUpdate(download_url, new_version)
	...
	local progress_msg = _toast(t("updater_downloading", new_version), 120)
	...
	local function doDownloadAndInstall()
		...
		if
			(saved_keys.gemini and saved_keys.gemini ~= "")
			or (saved_keys.chatgpt and saved_keys.chatgpt ~= "")
			or (saved_keys.deepseek and saved_keys.deepseek ~= "")
			or (saved_keys.claude and saved_keys.claude ~= "")
			or (saved_keys.custom1_key and saved_keys.custom1_key ~= "")
			or (saved_keys.custom2_key and saved_keys.custom2_key ~= "")
		then
			...
			if nfh then
				...
				if saved_keys.chatgpt and saved_keys.chatgpt ~= "" then
					content =
						content:gsub('chatgpt_api_key%s*=%s*""', 'chatgpt_api_key = "' .. saved_keys.chatgpt .. '"')
				end
				if saved_keys.deepseek and saved_keys.deepseek ~= "" then
					content =
						content:gsub('deepseek_api_key%s*=%s*""', 'deepseek_api_key = "' .. saved_keys.deepseek .. '"')
				end
				...
				if saved_keys.custom1_key and saved_keys.custom1_key ~= "" then
					content =
						content:gsub('custom1_api_key%s*=%s*""', 'custom1_api_key = "' .. saved_keys.custom1_key .. '"')
				end
				if saved_keys.custom1_endpoint and saved_keys.custom1_endpoint ~= "" then
					content = content:gsub(
						'custom1_endpoint%s*=%s*""',
						'custom1_endpoint = "' .. saved_keys.custom1_endpoint .. '"'
					)
				end
				if saved_keys.custom1_model and saved_keys.custom1_model ~= "" then
					content =
						content:gsub('custom1_model%s*=%s*""', 'custom1_model = "' .. saved_keys.custom1_model .. '"')
				end

				if saved_keys.custom2_key and saved_keys.custom2_key ~= "" then
					content =
						content:gsub('custom2_api_key%s*=%s*""', 'custom2_api_key = "' .. saved_keys.custom2_key .. '"')
				end
				if saved_keys.custom2_endpoint and saved_keys.custom2_endpoint ~= "" then
					content = content:gsub(
						'custom2_endpoint%s*=%s*""',
						'custom2_endpoint = "' .. saved_keys.custom2_endpoint .. '"'
					)
				end
				if saved_keys.custom2_model and saved_keys.custom2_model ~= "" then
					content =
						content:gsub('custom2_model%s*=%s*""', 'custom2_model = "' .. saved_keys.custom2_model .. '"')
				end
	...
	if ok_tr and Trapper and Trapper.dismissableRunInSubprocess then
		local completed, result = Trapper:dismissableRunInSubprocess(doDownloadAndInstall, progress_msg, function(res)
			handleInstallResult(res)
		end)
		...
		UIManager:show(ConfirmBox:new({
			...
			ok_callback = function()
				...
				if Device:canOpenLink() then
					Device:openLink(
						string.format("https://github.com/%s/%s/releases/latest", GITHUB_OWNER, GITHUB_REPO)
					)
				end
			end,
		}))
...
local function _doFetch(use_beta)
	...
	if not release then
		return { error = "parse error: " .. tostring(parse_err) }
	end
