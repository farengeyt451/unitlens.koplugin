	...
	require("ui/network/manager"):runWhenOnline(function()
		...
		UIManager:scheduleIn(0.3, function()
			...
			UIManager:scheduleIn(0.3, function()
				...
				local samples, chapter_titles =
					self.chapter_analyzer:getDetailedChapterSamples(self.ui, 100, 60000, limit_percent == 100)
				...
				local book_text =
					self.chapter_analyzer:getTextFromPageRange(self.ui, math.max(1, current_page - 1), end_page, 25000)

				local context_prefix = 'SEARCH TARGET: "'
					.. text
					.. '"\n'
...
function M:continueWithFetch(reading_percent, is_update, last_fetch_page, is_silent)
	...
	if cached and cached.timeline and #cached.timeline > 0 then
		...
		if current_len > cached_len then
			self:log(
				"XRayPlugin: timeline_event_len increased from "
					.. tostring(cached_len)
					.. " to "
					.. tostring(current_len)
	...
	if not is_silent then
		local ButtonDialog = require("ui/widget/buttondialog")
		local fetch_text = is_update and (self.loc:t("updating_ai", self.ai_provider or "AI") or "Updating X-Ray...")
	...
	UIManager:scheduleIn(0.5, function()
			...
			for _, entry in ipairs(candidate_chapters) do
				...
				if not found then
					if not first_missing_page or entry.page < first_missing_page then
						...
						self:log(
							"XRayPlugin: Repair mode active: recovering missing chapter '"
		...
		local book_text =
			self.chapter_analyzer:getTextForAnalysis(self.ui, 20000, nil, end_page_analysis, first_missing_page)
		...
		UIManager:scheduleIn(0, function()
			...
			result_file = settings_xray_dir
				.. "/bg_fetch_"
				.. tostring(os.time())
				.. "_"
				.. tostring(fetch_generation)
				.. ".json"
			...
			local function poll()
				if is_cancelled or self.destroyed then
					cancelActiveRequest(
						self.destroyed and "Fetch stopped because plugin was destroyed" or "Fetch cancelled"
					)
					return
				end
				if not self.ui or not self.ui.document then
...
function M:finalizeXRayData(final_book_data, title, author, book_text, is_update, is_silent, current_page)
		...
		for _, new_event in ipairs(incoming_timeline) do
			...
			for _, existing_event in ipairs(current_timeline) do
				...
				if new_norm == exist_norm then
					if
						new_event.page
						and existing_event.page
						and tonumber(new_event.page) == tonumber(existing_event.page)
					then
			...
			if not found then
				table.insert(current_timeline, new_event)
			end
		end

		self.timeline = {}
		for _, ev in ipairs(current_timeline) do
			table.insert(self.timeline, ev)
		end
		for _, ev in ipairs(prior_timeline) do
			table.insert(self.timeline, ev)
		end
	...
	if is_silent then
		self:log(
			string.format(
				"XRayPlugin: Silent merge complete - Chars: %d, Locs: %d, Events: %d, Cache: %s",
				#self.characters,
				#self.locations,
				#self.timeline,
...
function M:runPostFetchDuplicateCheck(title, author, reading_percent, is_silent)
	...
	if not self.ai_helper or not self.ai_helper.hasApiKey or not self.ai_helper:hasApiKey() then
		return
	end
	if self.ai_helper.settings and self.ai_helper.settings.auto_dupe_check_enabled == false then
		return
	end
	...
	local dup_book_text = self.chapter_analyzer:getTextForAnalysis(self.ui, 15000, nil, self.ui:getCurrentPage())
	...
	local function checkListAsync(list, list_name, entity_label, on_done)
		...
		local pid = self.ai_helper:findDuplicatesAsync(
			title,
			author,
			list,
			entity_label,
		...
		function(char_pairs)
			checkListAsync(
				...
				function(loc_pairs)
					...
					if not has_chars and not has_locs then
						return
					end
					...
					if is_silent then
						...
						self:log(
							"XRayPlugin: Stored "
								.. tostring(has_chars and #char_pairs or 0)
								.. " char and "
								.. tostring(has_locs and #loc_pairs or 0)
								.. " loc duplicate pairs for later review"
						)
					else
						UIManager:scheduleIn(0.5, function()
							if self.destroyed then
	...
	require("ui/network/manager"):runWhenOnline(function()
		...
		wait_msg = ButtonDialog:new({
			title = (self.loc:t("fetching_ai") or "Fetching AI...") .. "\n\n" .. (self.loc:t(
				"extracting_more_characters"
			) or "Extracting additional characters...") .. "\n\n" .. title,
			buttons = {
				{
					{
						text = self.loc:t("cancel") or "Cancel",
		...
		UIManager:scheduleIn(0.5, function()
			...
			if segment_size < pages_per_sample then
				segment_size = pages_per_sample
			end
			...
			for i = 0, num_samples - 1 do
				...
				if sample_start <= current_page then
					...
					local sample =
						self.chapter_analyzer:getTextFromPageRange(self.ui, sample_start, end_page, chars_per_sample)
					if sample and #sample > 100 then
						table.insert(text_parts, "[SECTION " .. (i + 1) .. "]\n" .. sample)
						self:log(
							"XRayPlugin: More chars sample "
								.. (i + 1)
								.. " pages "
								.. sample_start
								.. "-"
			...
			if is_cancelled or error_code == "USER_CANCELLED" then
				return
			end
			...
			local total_pages = self.ui
					and self.ui.document
					and self.ui.document.getPageCount
					and self.ui.document:getPageCount()
				or 1
			local reading_percent = total_pages and total_pages > 0 and math.floor((current_page / total_pages) * 100)
				or 100
			local spoiler_setting = self.ai_helper
					and self.ai_helper.settings
					and self.ai_helper.settings.spoiler_setting
				or "spoiler_free"
			if spoiler_setting == "full_book" then
				reading_percent = 100
			end
...
function M:fetchMoreTerms()
	require("ui/network/manager"):runWhenOnline(function()
		...
		local wait_msg =
			ButtonDialog:new({
				title = (self.loc:t("fetching_ai") or "Fetching AI...") .. "\n\n" .. (self.loc:t(
					"extracting_more_terms"
				) or "Extracting additional terms...") .. "\n\n" .. title,
				buttons = {
					{
						{
							text = self.loc:t("cancel") or "Cancel",
	...
	UIManager:scheduleIn(0.5, function()
		...
		self.author_info = {
			...
			description = sanitizeMetadata(
				author_data.author_bio or self.loc:t("msg_no_bio") or "No biography available."
			),
...
function M:mergeSeriesContext(cache_data, series_info)
	if not cache_data or not series_info then
		return
	end
	...
	for idx = 1, (series_info.index or 1) - 1 do
		...
		if book_data then
							...
							if matches then
								...
								if new_char.description and new_char.description ~= "" then
									...
									if not exist_desc:find(prefix, 1, true) then
										existing_char.description = prefix
			...
			if #events > 0 then
				local book_title = book_data.title
					or (cache_data.books and cache_data.books[idx] and cache_data.books[idx].title)
					or ""
				local label
	...
	local toc = self.ui and self.ui.document and self.ui.document.getToc and utils:flattenTOC(self.ui.document:getToc())
		or {}
...
function M:fetchSeriesContext(is_silent, init_wait_dialog, cancel_ref)
	...
	self:log(
		"XRayPlugin: Series: fetchSeriesContext starting for: title="
			.. tostring(title)
			.. ", author="
			.. tostring(author)
	)

	if init_wait_dialog and self.ai_helper then
		self.ai_helper:setTrapWidget(init_wait_dialog)
	end
	...
	if not series_info or not series_info.name or not series_info.index or series_info.index <= 1 then
		self:log(
			"XRayPlugin: Series: No series detected or current book is the first one in the series. series_info="
				.. (
					series_info
						and ("name=" .. tostring(series_info.name) .. ", index=" .. tostring(series_info.index))
					or "nil"
				)
		)
		closeInitWait()
	...
	self:log(
		"XRayPlugin: Series: Detected series="
			.. series_info.name
			.. ", index="
	...
	self:log(
		"XRayPlugin: Series: Needs to fetch "
			.. tostring(#missing_books)
			.. " missing books from AI. Running when online."
	)
	...
	require("ui/network/manager"):runWhenOnline(function()
		...
		local function fetchNext(step_idx)
			...
			self:log(
				"XRayPlugin: Series: Fetching AI context for book "
					.. tostring(current_book.index)
					.. " ("
					.. tostring(step_idx)
					.. "/"
					.. tostring(#missing_books)
					.. "): "
					.. tostring(current_book.title)
			)
			...
			UIManager:scheduleIn(0.5, function()
				...
				local prompt = self.ai_helper:createPrompt(
					current_book.title,
					current_book.author or author,
					context,
					"series_book_summary"
				)
				...
				if not result then
					self:log(
						"XRayPlugin: Series: Failed fetching book summary for "
							.. tostring(current_book.title)
							.. ", err_code="
							.. tostring(err_code)
							.. ", err_msg="
				...
				self:log(
					"XRayPlugin: Series: Fetched context for "
						.. tostring(current_book.title)
						.. ". Characters="
