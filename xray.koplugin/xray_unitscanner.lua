...
local function _circle_tile(diameter, grey)
	...
	local svg = string.format(
		'<svg width="%d" height="%d" viewBox="0 0 %d %d" xmlns="http://www.w3.org/2000/svg"><circle cx="%f" cy="%f" r="%f" fill="%s"/></svg>',
		diameter,
		diameter,
		diameter,
		diameter,
...
local function _checkSettingsChanged(self)
	...
	local disabled_types =
		table.concat(settings.unit_disabled_book_types or { "manga", "graphic_novel", "children", "poetry" }, ",")
	...
	local scan_changed = (state.direction ~= direction)
		or (state.dim_units ~= dim_units)
		or (state.cat_l ~= cat_l)
		or (state.cat_w ~= cat_w)
		or (state.cat_t ~= cat_t)
		or (state.cat_v ~= cat_v)
		or (state.cat_s ~= cat_s)
		or (state.cat_a ~= cat_a)

	local draw_changed = (state.enabled ~= enabled)
		or (state.underline_enabled ~= underline_enabled)
		or (state.book_type ~= book_type)
		or (state.disabled_types ~= disabled_types)
...
function M:_drawUnitUnderlines(bb)
	...
	if underline_style == "invisible" then
		return
	end
...
local function extend_span_start(doc, unit_start, num_val)
	if not doc or not unit_start or not num_val then
		return unit_start
	end
	...
	for i = 1, 4 do
		...
		if not ok or not prev or prev == cand then
			break
		end
		cand = prev
		...
		if ok2 and t then
			if t:find("[\r\n]") or t:find("%.%s") or t:find("%!%s") or t:find("%?%s") then
				break
			end
...
function M:loadUnitCache(resolved_dir)
	...
	if signature ~= current_sig then
		log(
			"loadUnitCache: Cache settings signature mismatch. File: ["
				.. tostring(signature)
				.. "], Current settings: ["
				.. tostring(current_sig)
				.. "]. Ignoring."
		)
		f:close()
		return false
	...
	for line in f:lines() do
		line = line:gsub("\r", "")
		local start_xp, end_xp, original, converted, category =
			line:match("^([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)$")
		if start_xp then
		...
		local ok_scan, err_scan = pcall(function()
			...
			if pat_word then
				...
				if #pat_word > MAX_REGEX_LEN then
					log(
						"scanBookForUnits: word pattern too large ("
							.. #pat_word
							.. " chars), skipping word pass safety check"
					)
					pat_word = nil
				end
			end
			...
			log(
				string.format(
					"scanBookForUnits: checkpoint B - post findAllText digit (took %.2fs), %d hits",
					t1 - t0,
					ok1 and hits1 and #hits1 or 0
				)
			)
			...
			if pat_word then
				...
				log(
					string.format(
						"scanBookForUnits: checkpoint D - post findAllText word (took %.2fs), %s hits",
						t2 - t1,
						tostring(ok2 and hits2 and #hits2 or 0)
					)
				)
			end
				...
				if vague then
					...
					if r1 and r2 then
						...
						if w1 and w2 then
							...
							if
								combined_val
								and not p:find("%s+to%s")
								and not p:find("%s+or%s")
								and not p:find("%s+and%s")
								and not p:find(",")
							then
								is_compound = true
							end
							...
							if not is_compound then
								...
								if val1 and val2 then
									is_range = true
									num_str = p:match("([%a%d%-]+%s*[,]?%s*(?:to|or|%-|and|,)%s*[%a%d%-]+)$")
										or (w1 .. " to " .. w2)
								end
							end
						end
				...
				if val or (val1 and val2) then
					...
					if u and enabled_cats[u.category] then
						...
						if is_range then
							...
							local conv_val1, conv_unit =
								xray_units.applySmartScaling(conv_raw1, u.category, u.std_target)
							...
							if conv_unit == "c" then
								conv_unit = "°C"
							elseif conv_unit == "f" then
								conv_unit = "°F"
							end

							conv_str = (is_vague and "≈" or "")
								.. xray_units.formatNumber(conv_val1, lang)
								.. "–"
								.. xray_units.formatNumber(conv_val2, lang)
								.. " "
								.. xray_units.pluralizeUnit(conv_val2, conv_unit)
						else
							local sign = ""
							...
							conv_str = xray_units.formatNumber(conv_val, lang)
								.. " "
								.. xray_units.pluralizeUnit(conv_val, conv_unit)
						end
						...
						if ok_t and real_text then
							...
							if not has_newline or not is_ambig[matched_alias] then
								...
								valid = true
								if not is_range then
									local lower_orig = original_text
										:gsub("−", "-")
										:gsub("–", "-")
										:gsub("—", "-")
										:lower()
										:gsub("\194\160", " ")
										:gsub("%s+", " ")
									local lower_num = num_str
										:gsub("^%s+", "")
										:gsub("%s+$", "")
										:lower()
										:gsub("\194\160", " ")
										:gsub("%s+", " ")
									local lower_match = hit.matched_text
										:lower()
										:gsub("\194\160", " ")
										:gsub("%s+", " ")
										:gsub("^%s+", "")
			...
			log(
				string.format(
					"scanBookForUnits: Lua processing took %.2fs, %d unit matches",
					t3 - t_start_lua,
					#xp_matches
				)
			)
...
function M:findUnitBoxAtPoint(x, y)
	if not self.unit_conversion_boxes then
		return nil
	end
	local tolerance = 12
	for _, box in ipairs(self.unit_conversion_boxes) do
		if
			x >= box.x - tolerance
			and x <= box.x + box.w + tolerance
			and y >= box.y - tolerance
			and y <= box.y + box.h + tolerance
		then
			return box
		end
	end
...
function M:mountTapHandler()
	if self._tapHandler_wrapped then
		return
	end
	local plugin = self
	...
	hl.onTap = function(hl_self, _, ges)
		if ges and plugin:_handleUnitTap(ges) then
			return true
		end
		if orig_tap then
...
function M:_handleUnitTap(ges)
	...
	for _, box in ipairs(self.unit_conversion_boxes) do
		if tx >= box.x and tx <= box.x + box.w and ty >= box.y - 6 and ty <= box.y + box.h + 6 then
