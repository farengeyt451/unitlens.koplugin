...
local sanitize_utf8 = function(s)
	return AIHelper:sanitize_utf8(s)
end
...
function AIHelper:resolveModel(provider_id, model)
	...
	if provider_id == "custom1" or provider_id == "custom2" then
		if not model or model == provider_id then
			...
			if model then
				self:log(
					"AIHelper: WARNING: no model configured for "
						.. provider_id
						.. " (set "
						.. provider_id
						.. "_model in xray_config.lua or the API Keys menu)"
				)
				return nil
			end
		end
...
function AIHelper:makeRequest(url, headers, request_body, timeout, maxtime)
	...
	local function performRequest()
		...
		socketutil_req:set_timeout(timeout, maxtime)
		local response_body = {}
		local request = {
			url = url,
			method = "POST",
			headers = headers or {},
			source = ltn12_req.source.string(request_body or ""),
		...
		if response_headers and response_headers["content-length"] then
			...
			if clen and #response_text < clen then
				return nil, "error_incomplete", "Incomplete response"
			end
		end
		if ok == nil and (code == "timeout" or tostring(code):find("timeout")) then
	...
	if self.trap_widget and Trapper and Trapper.dismissableRunInSubprocess then
		local completed, ok, code, response_text, status =
			Trapper:dismissableRunInSubprocess(performRequest, self.trap_widget)
	...
	for _, ai in ipairs({ primary, secondary }) do
		...
		if config and config.api_key and config.api_key ~= "" then
			...
			if ai.provider == "gemini" then
				...
				local system_instruction_text = self.prompts and self.prompts.system_instruction
					or "Return valid JSON ONLY."
				...
				then
					if model:find("gemini%-3") then
						...
						if (model:find("1%.5") or model:find("2%.0%-flash")) and budget > 2000 then
							self:log(
								string.format(
									"AIHelper: Constrained Gemini %s detected - capping thinking budget from %d to 2000 (limit=8192)",
									model,
									budget
								)
							)
			...
			elseif self:isAnthropic(ai.provider, config.endpoint) then
				...
				local is_native_anthropic = (ai.provider == "claude")
					or (config.endpoint or ""):find("api.anthropic.com")
				...
				local system_instruction_text = (
					self.prompts and self.prompts.system_instruction or "Return valid JSON ONLY."
				...
				if model:find("sonnet") or model:find("opus") or model:find("haiku") then
					...
					if current_effort then
						...
						if model:find("haiku") and budget > 2000 then
							self:log(
								string.format(
									"AIHelper: Haiku detected - capping thinking budget from %d to 2000 (haiku max_tokens=8192)",
									budget
								)
							)
							budget = 2000
				...
				if
					ai.provider == "deepseek" and (model:find("reasoner") or model:find("v4") or model:find("deepseek"))
				then
				...
				if
					(ai.provider == "chatgpt" or ai.provider == "custom1" or ai.provider == "custom2")
					and (model:find("^gpt%-5") or model:find("^o[13]"))
				then
					local current_effort = self.settings and self.settings.reasoning_effort
					if current_effort then
						...
						if current_effort == "high" then
							...
							self:log(
								string.format(
									"AIHelper: OpenAI %s at %s effort - dropping json_object mode, raising max_completion_tokens to 65000",
									model,
									current_effort
								)
							)
				...
				if ai.provider == "custom1" or ai.provider == "custom2" then
					...
					if is_reasoning then
						...
						self:log(
							string.format(
								"AIHelper: Custom slot %s marked as reasoning model - using 32000 token ceiling",
								ai.provider
							)
						)
					end
...
function AIHelper:hasApiKey()
	if self.providers.gemini and self.providers.gemini.api_key and self.providers.gemini.api_key ~= "" then
		return true
	end
	if self.providers.chatgpt and self.providers.chatgpt.api_key and self.providers.chatgpt.api_key ~= "" then
		return true
	end
	if self.providers.deepseek and self.providers.deepseek.api_key and self.providers.deepseek.api_key ~= "" then
		return true
	end
	if self.providers.claude and self.providers.claude.api_key and self.providers.claude.api_key ~= "" then
		return true
	end
	if self.providers.custom1 and self.providers.custom1.api_key and self.providers.custom1.api_key ~= "" then
		return true
	end
	if self.providers.custom2 and self.providers.custom2.api_key and self.providers.custom2.api_key ~= "" then
		return true
	end
	return false
end
...
function AIHelper:_reapAsyncChild(pid)
	if not pid or self._async_child_pid ~= pid then
		return false
	end
...
function AIHelper:_scheduleAsyncChildReap(pid)
	...
	if self._async_children_to_reap[pid] then
		return true
	end
		...
		elseif payload ~= "" then
			...
			if stream_format == "openai" then
				...
				if
					...
					and type(event.content_block.text) == "string"
				then
				...
				elseif
					...
					and type(event.delta.text) == "string"
				then
...
function AIHelper:makeRequestAsync(request_params, result_file)
	if self._async_child_pid and not self:_reapAsyncChild(self._async_child_pid) then
		self:log(
			"AIHelper: Cannot start async request while PID " .. tostring(self._async_child_pid) .. " is still active"
		)
		return false
	end
	...
	local function child_logic(pid, write_fd)
		local child_ok, child_err = pcall(function()
			...
			if request_params.url then
				requests = { request_params }
			end -- Handle single request fallback

			local success_found = false
			for i, req in ipairs(requests) do
				self:log(
					string.format(
						"AIHelper Child: Sending request %d/%d to %s (%s)",
						i,
						#requests,
						req.provider,
				...
				if code_num == 200 then
								...
								if not valid_json then
									...
									if first_brace then
										...
										if repair_ok and repair_data then
											...
											self:log(
												"AIHelper Child: Quick repair unsuccessful; handing off to main thread for advanced parsing"
											)
							...
							if content then
								...
								if not valid_json then
									...
									if first_brace then
										...
										if repair_ok and repair_data then
											self:log(
												"AIHelper Child: fixTruncatedJSON succeeded for ChatGPT/"
													.. req.provider
											)
											local synthetic = json_req.encode({
												choices = {
							...
							if content then
								...
								if not valid_json then
									...
									if first_brace then
										...
										if repair_ok and repair_data then
											self:log(
												"AIHelper Child: fixTruncatedJSON succeeded for Anthropic / "
													.. req.provider
											)
					...
					if valid_json then
						...
						self:log(
							string.format(
								"AIHelper Child: Provider %s returned 200 but JSON is invalid/truncated. Trying fallback.",
								req.provider
							)
						)
					...
					self:log(
						string.format("AIHelper Child: Provider %s failed with code %s", req.provider, tostring(code))
					)
...
function AIHelper:checkAsyncResult(result_file, expected_pid)
	if expected_pid then
		local owns_expected_request = self._async_child_pid == expected_pid and self._async_result_file == result_file
	...
	if not first_newline then
		return false, "error_parse", "Malformed async result (empty or no newline)"
	end
	...
	if not second_newline then
		return false, "error_parse", "Malformed async result (no provider line)"
	end
	...
	if not success then
		return false, "error_parse", "JSON decode failed"
	end
	...
	elseif
		provider == "claude"
		or self:isAnthropic(provider, self.providers[provider] and self.providers[provider].endpoint)
	then
...
function AIHelper:cancelAsyncChild(expected_pid)
	...
	if expected_pid and expected_pid ~= pid then
		self:log(
			"AIHelper: Refusing to cancel stale PID " .. tostring(expected_pid) .. "; active PID is " .. tostring(pid)
		)
		return false
	end
	...
	local termination_requested = self:_terminateAsyncProcess(pid, self._async_child_uses_process_group)
...
function AIHelper:loadConfig()
	...
	if config then
		...
		if config.gemini_primary_model then
			self.providers.gemini.primary_model = config.gemini_primary_model
		end
		if config.gemini_secondary_model then
			self.providers.gemini.secondary_model = config.gemini_secondary_model
		end
		...
		if config.chatgpt_model then
			self.providers.chatgpt.model = config.chatgpt_model
		end
		if config.deepseek_api_key then
			self.providers.deepseek.api_key = config.deepseek_api_key
			self.config_keys.deepseek = config.deepseek_api_key
		end
		if config.claude_api_key then
			self.providers.claude.api_key = config.claude_api_key
			self.config_keys.claude = config.claude_api_key
		end
		...
		for _, slot in ipairs({ "custom1", "custom2" }) do
			if config[slot .. "_api_key"] then
				self.providers[slot].api_key = config[slot .. "_api_key"]
				self.config_keys[slot] = config[slot .. "_api_key"]
			end
			if config[slot .. "_endpoint"] then
				self.providers[slot].endpoint = config[slot .. "_endpoint"]
			end
			if config[slot .. "_model"] then
				self.providers[slot].model = config[slot .. "_model"]
			end
			if config[slot .. "_format"] then
				self.providers[slot].format = config[slot .. "_format"]
			end
		end
	end
end
...
function AIHelper:loadSettings()
	...
	if not settings.gemini_primary_model then
		settings.gemini_primary_model = self.providers.gemini.primary_model
	end
	if not settings.gemini_secondary_model then
		settings.gemini_secondary_model = self.providers.gemini.secondary_model
	end
	if not settings.chatgpt_model then
		settings.chatgpt_model = self.providers.chatgpt.model
	end

	-- Description length defaults (chars per entity type)
	if not settings.char_desc_len then
		settings.char_desc_len = 200
	end
	if not settings.loc_desc_len then
		settings.loc_desc_len = 100
	end
	if not settings.timeline_event_len then
		settings.timeline_event_len = 80
	end
	if not settings.hist_fig_bio_len then
		settings.hist_fig_bio_len = 100
	end
	if not settings.term_def_len then
		settings.term_def_len = 100
	end
	if not settings.default_book_mode then
		settings.default_book_mode = "auto"
	end
	if not settings.terms_visibility then
		settings.terms_visibility = "always"
	end
	if settings.series_context_enabled == nil then
	...
	if type(settings.primary_ai) ~= "table" then
		...
		if def_prov == "gemini" then
			settings.primary_ai =
				{ provider = "gemini", model = settings.gemini_primary_model or DEFAULT_AI.primary.model }
		else
	...
	if type(settings.secondary_ai) ~= "table" then
		settings.secondary_ai =
			{ provider = "gemini", model = settings.gemini_secondary_model or DEFAULT_AI.secondary.model }
		migrated = true
	end
	...
	local function migrate_deepseek_model(ai_slot)
		if type(settings[ai_slot]) == "table" and settings[ai_slot].provider == "deepseek" then
			...
			if old and deepseek_model_map[old] then
				self:log(
					string.format(
						"AIHelper: Migrating DeepSeek model '%s' -> '%s' in %s",
						old,
						deepseek_model_map[old],
						ai_slot
					)
				)
	...
	for _, slot in ipairs({ "custom1", "custom2" }) do
		...
		if settings[slot .. "_endpoint"] then
			self.providers[slot].endpoint = settings[slot .. "_endpoint"]
		end
		if settings[slot .. "_model"] then
			self.providers[slot].model = settings[slot .. "_model"]
		end
		if settings[slot .. "_format"] then
			self.providers[slot].format = settings[slot .. "_format"]
		end
	end
	...
	for _, ai_slot in ipairs({ "primary_ai", "secondary_ai" }) do
		...
		if
			type(ai) == "table"
			and (ai.provider == "custom1" or ai.provider == "custom2")
			and (ai.model == nil or ai.model == "" or ai.model == ai.provider)
		then
			...
			if slot_model and slot_model ~= "" and ai.model ~= slot_model then
				self:log(
					string.format(
						"AIHelper: Replacing %s placeholder model '%s' with configured '%s'",
						ai_slot,
						tostring(ai.model),
						slot_model
					)
...
function AIHelper:loadLanguage()
	...
	if self.current_language ~= "en" then
		...
		if ok_loc and type(loc_prompts) == "table" then
			for k, v in pairs(loc_prompts) do
				self.prompts[k] = v
			end
		end
	end
...
function AIHelper:createPrompt(title, author, context, section_name, targeted_word)
	...
	if context then
		if context.series then
			enhanced_title = enhanced_title .. " | Series: " .. context.series
		end
		...
		if section_name == "comprehensive_xray" or section_name == "single_word_lookup" then
			...
			if context.chapter_samples then
				extra_context = extra_context .. "\n\nCHAPTER SAMPLES:\n" .. context.chapter_samples
			end
		end
		if context.annotations then
			extra_context = extra_context .. "\n\nUSER HIGHLIGHTS:\n" .. context.annotations
		end
		if context.book_type then
			extra_context = extra_context
				.. "\n\nCRITICAL: The book type is already known to be: "
				.. context.book_type
				.. '. You MUST declare "book_type" as "'
				.. context.book_type
				.. '" at the JSON root and follow the extraction rules for '
				.. context.book_type
				.. " books."
		end
		...
		local merge_instructions = "\n\nMERGE MODE INSTRUCTIONS:\nYou are UPDATING an existing X-Ray.\n- For entities (Characters, Locations, Historical Figures) that already exist, synthesize a completely rewritten, cohesive summary combining the EXISTING KNOWLEDGE with any new information found in the text.\n- Write a solid summary that is not repetitive.\n- Descriptions MUST NOT exceed "
			.. tostring(_merge_char_len)
			.. " characters.\n- If there is no new information, return the existing description (or a refined version of it under "
			.. tostring(_merge_char_len)
			.. " characters)."
		...
		if context.existing_characters and #context.existing_characters > 0 then
						...
						if c.aliases then
							for _, alias in ipairs(c.aliases) do
								if
									type(alias) == "string"
									and #alias > 1
									and sample_lower:find(alias:lower(), 1, true)
								then
									found_in_sample = true
									break
								end
			...
			if #existing_lines > 0 then
				if not has_merge_data then
					extra_context = extra_context .. merge_instructions
					has_merge_data = true
				end
				extra_context = extra_context
	...
	elseif section_name == "prior_book_list" then
		...
		success, final_prompt = pcall(
			string.format,
			template,
			context.series_name or "Unknown",
			idx,
			enhanced_title,
			enhanced_author,
			idx - 1
	...
	elseif section_name == "series_book_summary" then
		local idx = context and context.index or 1
		success, final_prompt =
			pcall(string.format, template, enhanced_title, enhanced_author, idx, context.series_name or "Unknown")
	else
		success, final_prompt = pcall(
			string.format,
			template,
	...
	if not success then
		final_prompt = string.format("Extract %s data.", section_name)
	end
	...
	do
		...
		elseif tl_len <= 150 then
			...
			tl_example =
				"The hero escapes the burning city, pursued by guards, and reunites with companions at the river crossing, where they plan their next move against the antagonist."
		else
			tl_guidance =
				"Write a rich, full narrative description including character actions, key events, and their consequences."
			tl_example =
				"The hero escapes the burning city under cover of darkness, pursued by the king's guards. After a harrowing chase, he reunites with companions at the river crossing, where they learn the antagonist has seized the eastern fortress and begin planning a counterattack."
		end
	...
	if #final_prompt ~= before_len then
		self:log(
			string.format("AIHelper: sanitize_utf8 stripped %d invalid byte(s) from prompt", before_len - #final_prompt)
		)
	end
	return final_prompt
end
...
function AIHelper:callGemini(prompt, config, current_model)
	...
	local ok, code, response_text, status = self:makeRequest(
		url,
	...
	self:log(
		"AIHelper: ["
			.. current_model
			.. "] Response received ("
			.. (response_text and #response_text or 0)
			.. " bytes)"
	)
	...
	elseif code_num == 429 then
		return nil, "error_quota", "Quota Exceeded (429)"
	elseif code_num == 503 then
		self:log("AIHelper: 503 Overload")
		socket.sleep(2)
	else
		local error_detail = "HTTP " .. tostring(code_num or code or "Unknown")
		if response_text then
...
function AIHelper:callChatGPT(prompt, config, current_model)
	...
	if legacy_models[model] then
		local err = "Model '"
			.. model
			.. "' does not support JSON mode. Please use gpt-4o, gpt-4-turbo, or gpt-4o-mini."
	...
	local ok, code, response_text =
		self:makeRequest(config.endpoint or "https://api.openai.com/v1/chat/completions", headers, request_body)
...
-- Backward compatibility for internal calls
local fixTruncatedJSON = function(s)
	return AIHelper:fixTruncatedJSON(s)
end
...
function AIHelper:parseAIResponse(text)
	...
	if success and data then
		return self:validateAndCleanData(normalizeKeys(data))
	end
...
function AIHelper:validateAndCleanData(data)
	...
	if data.is_valid ~= nil or data.duplicate_pairs ~= nil or data.DuplicatePairs ~= nil then
		return data
	end
	...
	for _, c in ipairs(chars) do
		if type(c) == "table" then
			table.insert(valid_chars, {
				name = ensureString(
					c.name or c.full_formal_name or c.full_name or c.formal_name or c.Name,
					strings.unnamed_character
				),
...
function AIHelper:setCustomAPIConfig(slot, key, endpoint, model, format)
	...
	if format then
		self.providers[slot].format = format
	end
	self.providers[slot].ui_key_active = true
	local s_updates = {
		[slot .. "_api_key"] = key,
...
function AIHelper:findDuplicates(title, author, entities, entity_type_label, reading_percent, book_text)
	...
	if not template then
		return nil, "no_prompt", "find_duplicates prompt missing"
	end
...
function AIHelper:findDuplicatesAsync(
	title,
	author,
	entities,
	entity_type_label,
	...
	local requests, error_code, error_msg = self:buildComprehensiveRequest(nil, nil, nil, prompt)
	if not requests then
		return nil, error_code or "error_build", error_msg or "Failed to build request"
	end
...
function AIHelper:detectBookTypeAsync(title, author, series, description, result_file)
	...
	if not template then
		return nil, "no_prompt", "book_type_detect prompt missing"
	end

	local prompt =
		string.format(template, title or "Unknown", author or "Unknown", series or "None", description or "None")
	prompt = self:sanitize_utf8(prompt)
